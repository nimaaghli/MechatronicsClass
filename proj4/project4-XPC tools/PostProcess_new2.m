...
n=6E5;
%n=20000;
%plot([1:n],SQUARE(1:n,1),[1:n],SQUARE(1:n,2));
...
yS = [SQUARE(:,1) SQUARE(:,3) SQUARE(:,4)]; %outputs
uS = [SQUARE(:,2)]; %inputs

yT = [TRIANGLE(:,1) TRIANGLE(:,3) TRIANGLE(:,4)]; %outputs
uT = [TRIANGLE(:,2)]; %inputs

y = [PRBS(:,1) PRBS(:,3) PRBS(:,4)]; %outputs
u = [PRBS(:,2)]; %inputs
...
set(mdata, 'OutputName', {'Current','Angular position', 'Angular velocity'});
set(mdata, 'OutputUnit', {'A','rad', 'rad/s'});
set(mdata, 'Tstart', 0, 'TimeUnit', 's');

set(vdata, 'InputName', 'Voltage', 'InputUnit', 'V');
set(vdata, 'OutputName', {'Current','Angular position', 'Angular velocity'});
set(vdata, 'OutputUnit', {'A','rad', 'rad/s'});
set(vdata, 'Tstart', 0, 'TimeUnit', 's');

set(vSdata, 'InputName', 'Voltage', 'InputUnit', 'V');
set(vSdata, 'OutputName', {'Current','Angular position', 'Angular velocity'});
set(vSdata, 'OutputUnit', {'A','rad', 'rad/s'});
set(vSdata, 'Tstart', 0, 'TimeUnit', 's');

set(vTdata, 'InputName', 'Voltage', 'InputUnit', 'V');
set(vTdata, 'OutputName', {'Current','Angular position', 'Angular velocity'});
set(vTdata, 'OutputUnit', {'A','rad', 'rad/s'});
...
%y = [PRBS(:,3)*2*pi() PRBS(:,4)*2*pi()]; %outputs
%y = [SQUARE(:,2) SQUARE(:,3) SQUARE(:,4)]; %current, theta, thetad
%u = [SQUARE(:,1)]; %inputs
%y=[Pos/16*2*pi() Vel*2*pi()/60];

%mean(SQUARE(2000:4000,2))
